    ...
    def to_batch(self, data:list, 
        ...
        for batch_idx, index_list in enumerate(batch_index_list):
            ...
            for item in item_list:
                if prompt_data is not None:
                    # 确保所有参与连接的张量都在同一设备上
                    prompt_bert_features = prompt_data["bert_features"].to(dtype=precision, device=device)
                    item_bert_features = item["bert_features"].to(dtype=precision, device=device)
                    
                    # 进行连接操作
                    all_bert_features = torch.cat([prompt_bert_features, item_bert_features], dim=1)
                    
                    all_phones = torch.LongTensor(prompt_data["phones"] + item["phones"]).to(device)
                    phones = torch.LongTensor(item["phones"]).to(device)
                else:
                    # 如果没有 prompt_data，确保 item["bert_features"] 在目标设备上
    ...
    def run(self, inputs:dict):
        ...
        if not return_fragment:
            ...
            def make_batch(batch_texts):
                ...
                for text in tqdm(batch_texts, disable=True): # disable=True 不显示进度条
